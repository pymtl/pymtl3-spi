...
#include "obj_dir_FPURetimed_noparam/VFPURetimed_noparam.h"
#include "stdio.h"
#include "stdint.h"
#include "verilated.h"
...
#if VLINETRACE
#include "obj_dir_FPURetimed_noparam/VFPURetimed_noparam__Syms.h"
#include "svdpi.h"
#endif
...
extern "C" {
  ...
  #if VLINETRACE
  void trace( VFPURetimed_noparam_t *, char * );
  #endif
...
VFPURetimed_noparam_t * create_model( const char *vcd_filename ) {

  VFPURetimed_noparam_t * m;
  VFPURetimed_noparam   * model;

  Verilated::randReset( 0 );
  Verilated::randSeed( 0 );

  m     = (VFPURetimed_noparam_t *) malloc( sizeof(VFPURetimed_noparam_t) );
  model = new VFPURetimed_noparam();
  ...
  m->clk = &model->clk;
  m->req_msg = model->req_msg;
  m->req_rdy = &model->req_rdy;
  m->req_val = &model->req_val;
  m->resp_msg = model->resp_msg;
  m->resp_rdy = &model->resp_rdy;
  m->resp_val = &model->resp_val;
...
void destroy_model( VFPURetimed_noparam_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  VFPURetimed_noparam * model = (VFPURetimed_noparam *) m->model;
...
void comb_eval( VFPURetimed_noparam_t * m ) {
...
void seq_eval( VFPURetimed_noparam_t * m ) {
...
#if VLINETRACE
void trace( VFPURetimed_noparam_t * m, char* str ) {
  ...
  svSetScope( &model->__VlSymsp->__Vscope_FPURetimed_noparam__v );
