...
#     # Variables
#     s.regs = [ RegEnRst(s.nbits_out) for _ in range(s.num_regs) ]

#     s.counter = RegEnRst(clog2(s.num_regs)+1) # the +1 is because we count up to s.num_regs e.g. if num_regs=2 then counter must go from 0->1->2
#     s.count_next = Wire(clog2(s.num_regs)+1)
#     s.do_increment = Wire(1)

#     s.transaction_val = RegEnRst(1)

#     # Mux
#     s.reg_mux = Mux(s.nbits_out, s.num_regs)

#     # Assigns
#     s.disassem_ifc.req.rdy   //= lambda: (s.transaction_val.out == 0)
#     s.disassem_ifc.resp.val  //= lambda: (s.transaction_val.out == 1)
...
#         s.transaction_val.en @= (s.disassem_ifc.req.val & s.disassem_ifc.req.rdy) | ((s.counter.out == (s.num_regs-1)) & s.disassem_ifc.resp.rdy)
#         s.transaction_val.in_ @=  (s.disassem_ifc.req.val & s.disassem_ifc.req.rdy)

#         s.do_increment @= s.disassem_ifc.resp.val & s.disassem_ifc.resp.rdy

#         s.count_next   @= 0 if (s.counter.out == (s.num_regs-1)) & s.disassem_ifc.resp.rdy else (s.counter.out + 1 if s.do_increment else s.counter.out)
#         s.counter.in_  @= s.count_next
#         s.counter.en   @= 1

#         s.regs[s.num_regs-1].in_ @= zext(s.disassem_ifc.req.msg[ (s.nbits_out*s.num_regs-s.nbits_out) : s.nbits_in ], s.nbits_out)  # holds MSb, zext to fit register size 
#         s.regs[s.num_regs-1].en  @= s.disassem_ifc.req.val & s.disassem_ifc.req.rdy

#         for i in range(s.num_regs):
#             s.reg_mux.in_[i] @= s.regs[i].out 

#         s.reg_mux.sel @= trunc(s.num_regs - s.counter.out - 1, s.reg_bits)

#         for i in range(s.num_regs-1):
#             s.regs[i].in_ @= s.disassem_ifc.req.msg[s.nbits_out*(i) : (s.nbits_out*i + s.nbits_out) ]
#             s.regs[i].en  @= s.disassem_ifc.req.val & s.disassem_ifc.req.rdy
