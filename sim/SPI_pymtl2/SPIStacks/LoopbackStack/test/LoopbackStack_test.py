...
from ..LoopbackStack  import LoopbackStack
...
class TestHarness( Component ):
  ...
  def construct( s, pack_size, flow_control, queue_size ):

    # Instantiate models

    s.src             = stream.SourceRTL( mk_bits(pack_size) )
    s.sink            = stream.SinkRTL  ( mk_bits(pack_size) )
    s.master          = SPIMaster       ( pack_size, 4 )
    s.loopback_stack  = LoopbackStack   ( pack_size, queue_size )

    # Connect
    
    s.src.send.val //= s.master.req_val
    s.src.send.rdy //= s.master.req_rdy
    s.src.send.msg //= s.master.req_msg
    
    s.sink.recv.val //= s.master.resp_val
    s.sink.recv.rdy //= s.master.resp_rdy
    s.sink.recv.msg //= s.master.resp_msg
    
    
    s.master.cs     //= s.loopback_stack.cs
    s.master.sclk   //= s.loopback_stack.sclk
    s.master.mosi   //= s.loopback_stack.mosi
    s.master.miso   //= s.loopback_stack.miso
